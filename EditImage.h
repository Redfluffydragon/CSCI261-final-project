...
private:
    ...
    unsigned int height; // The current height of the image in pixels
    unsigned int width; // The current width of the image in pixels
    ...
    // Always the original dimensions of the image
    ...
    void writePixel(unsigned int from,
                    unsigned int to); // Write a pixel from the read vector to the write vector at the positions specified
...
public:
    ...
    void
    rotate(const float &degrees); // Rotate the sprite and set the rotation data member of the image from where it is currently
