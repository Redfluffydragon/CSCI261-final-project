...
int main() {
    ...
    // Create EditImage object
    EditImage image; // Automatically loads a default image file
    ...
    while (window.isOpen()) {
        window.clear(Color::White); // clear the contents of the old frame by setting the window to black
            ...
            if (event.type == Event::Closed) { // if event type is a closed event
                ...
                if (resetBtn.isWithin(Mouse::getPosition(window))) {
                    ...
                    while (fileWindow.isOpen()) {
                        ...
                        fileWindow.draw(message); // Draw the informational message

                        fileWindow.draw(showFilename); // Draw the new file name

                        fileWindow.display(); // Update the window
                        ...
                        while (fileWindow.pollEvent(fileEvent)) {
                            ...
                            if (fileEvent.type == Event::Closed) { // if event type is a closed event
                            ...
                            } else if (fileEvent.type ==
                                       Event::Resized) { // Make it so things don't stretch when the window is resized
                                FloatRect visibleArea(0, 0, fileEvent.size.width, fileEvent.size.height);
                                fileWindow.setView(View(visibleArea));
                            } else if (fileEvent.type ==
                                       Event::TextEntered) { // If text is typed, add it to the file name for display
                                if (fileEvent.text.unicode < 128) { // Only basic characters
                                    ...
                                    } else if (fileEvent.text.unicode ==
                                               13) { // Enter closes the window without resetting the file name, so it will try to load the new file
                                        fileWindow.close();
                                    ...
                                    showFilename.setString(
                                            filename.empty() ? "File name goes here (start typing)" : filename);
                                }
                            }
                        }
                    }
